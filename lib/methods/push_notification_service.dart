...
class PushNotificationService {
  ...
  static sendNotificationToSelectedDriver(
    ...
    // Ensure pickUpLocation and dropOffLocation are not null
    if (appInfo.pickUpLocation == null || appInfo.dropOffLocation == null) {
      print('Error: pickUpLocation or dropOffLocation is null.');
      return; // Exit the function early to prevent further errors
    }

    String pickUpAddress = appInfo.pickUpLocation!.placeName.toString();
    String dropOffDestinationAddress =
        appInfo.dropOffLocation!.placeName.toString();

    // Ensure tripID is not null or empty
    if (tripID.isEmpty) {
      print('Error: tripID is null or empty.');
      return; // Exit the function early to prevent further errors
    }
